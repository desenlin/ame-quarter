...
async function check(mode){
  ...
  assert.equal($('#close-scene'),null);assert.equal($('[data-view="scene"]').textContent,'Explore 3-D Map');
  assert.equal($('#scene-wrap').hidden,false);assert.equal($('#plan-wrap').hidden,true);assert.equal($('[data-view="scene"]').getAttribute('aria-pressed'),'true');
  assert($('#scene-host iframe'),'3-D must open without a click');assert.equal($('#scene-host iframe').loading,'lazy');assert.equal($('#visit-unit').disabled,true);
  function mockDistrict(){
    const frame=$('#scene-host iframe'),child=frame.contentDocument,root=child.createElement('div');root.id='ame-district';root.dataset.sceneStatus='loading';
    if(!child.documentElement)child.append(child.createElement('html'));
    (child.body||child.documentElement).append(root);root.__district={suspended:false,setSuspended(v){this.suspended=v;}};
    frame.dispatchEvent(new w.Event('load'));return root;
  }
  const initialDistrict=mockDistrict();initialDistrict.dataset.sceneStatus='ready';await Promise.resolve();assert.equal($('#visit-unit').disabled,false);
  $('[data-page="leases"]').click();assert.equal(initialDistrict.__district.suspended,true);
  $('[data-page="property"]').click();assert.equal(initialDistrict.__district.suspended,false);
  $('[data-view="plan"]').click();assert.equal($('#scene-host iframe'),null);assert.equal(initialDistrict.__district.suspended,true);assert.equal($('#plan-wrap').hidden,false);
  ...
  const oldDistrict=$('#scene-host iframe').contentDocument.getElementById('ame-district');
  $('[data-view="plan"]').click();assert.equal($('#scene-host iframe'),null);assert.equal($('#plan-wrap').hidden,false);
  $('[data-view="scene"]').click();assert.equal($('#scene-quality').value,'full');assert.equal($('#lease-title').textContent,'Tsuki Books');
  const failedDistrict=mockDistrict();oldDistrict.dataset.sceneStatus='unavailable';await Promise.resolve();assert.equal($('#scene-wrap').hidden,false,'Detached scene cannot change the active view');
  failedDistrict.dataset.sceneStatus='unavailable';await Promise.resolve();assert.equal($('#scene-host iframe'),null);assert.equal($('#plan-wrap').hidden,false);assert.equal($('#scene-notice').hidden,false);assert.equal($('[data-view="plan"]').getAttribute('aria-pressed'),'true');
  assert.equal($('#lease-title').textContent,'Tsuki Books');assert.equal($('#replace-downtime').value,'4');
  $('[data-view="scene"]').click();assert.equal($('#scene-notice').hidden,true);const retryDistrict=mockDistrict();retryDistrict.dataset.sceneStatus='ready';await Promise.resolve();assert.equal($('#visit-unit').disabled,false);assert.equal($$('#scene-host iframe').length,1);
  $('[data-view="plan"]').click();assert.equal($('#scene-host iframe'),null);
  ...
  console.log('PASS ('+mode+'): automatic 3-D, readiness/pause, plan teardown, stale-frame protection, fallback/retry, new-tab graphics settings, lease selection and cash-flow interactions');
  w.close();
}
